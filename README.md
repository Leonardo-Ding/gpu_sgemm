...
We first write a cuda-c level code in matrixMul_cuda dir. Firstly, this is a 'NT'(C = A*B, A is a column-majored matrix, B is a row-majored matrix) format doing gemm operation. Secondly, ouput C is 64x64 tile format, input A and B is 64*8 format, and we use 64 cuda threads to do it.
...
2. This is only a version of matrix multiply for C = A*B and not a complete gemm version for C = alpha*A*B+beta*C,
